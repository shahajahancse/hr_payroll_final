...
ajaxRequest.onreadystatechange = function(){
	if(ajaxRequest.readyState == 4){
		...
		dept_idname_add_deduct = alldata[0].split("===");
		dept_id_add_deduct = dept_idname_add_deduct[0].split("***");
	    dept_name_add_deduct = dept_idname_add_deduct[1].split("***");
				
		document.add_deduction.dept_add_deduct.options.length=0;
		document.add_deduction.dept_add_deduct.options[0]=new Option("Select","Select", true, false);
		for (i=0; i<dept_id_add_deduct.length; i++){
			document.add_deduction.dept_add_deduct.options[i+1]=new Option(dept_name_add_deduct[i],dept_id_add_deduct[i], false, false);

		}
				
		sec_idname_add_deduct = alldata[1].split("===");
		sec_id_add_deduct = sec_idname_add_deduct[0].split("***");
		sec_name_add_deduct = sec_idname_add_deduct[1].split("***");
	 		
		document.add_deduction.section_add_deduct.options.length=0;
		document.add_deduction.section_add_deduct.options[0]=new Option("Select","Select", true, false); 
		for (i=0; i<sec_id_add_deduct.length; i++){
			//alert(sec_name[i]);
			document.add_deduction.section_add_deduct.options[i+1]=new Option(sec_name_add_deduct[i],sec_id_add_deduct[i], false, false);

		}
		
		
		line_idname_add_deduct = alldata[2].split("===");
		line_id_add_deduct = line_idname_add_deduct[0].split("***");
		line_name_add_deduct = line_idname_add_deduct[1].split("***");
		
		document.add_deduction.line_add_deduct.options.length=0;
		document.add_deduction.line_add_deduct.options[0]=new Option("Select","Select", true, false); 
		for (i=0; i<line_id_add_deduct.length; i++){
			document.add_deduction.line_add_deduct.options[i+1]=new Option(line_name_add_deduct[i],line_id_add_deduct[i], false, false);

		}
		
		
		desig_idname_add_deduct = alldata[3].split("===");
		desig_id_add_deduct = desig_idname_add_deduct[0].split("***");
		desig_name_add_deduct = desig_idname_add_deduct[1].split("***");
		
		document.add_deduction.desig_add_deduct.options.length=0;
		document.add_deduction.desig_add_deduct.options[0]=new Option("Select","Select", true, false); 
		for (i=0; i<desig_id_add_deduct.length; i++){
			document.add_deduction.desig_add_deduct.options[i+1]=new Option(desig_name_add_deduct[i],desig_id_add_deduct[i], false, false);
...
ajaxRequest.onreadystatechange = function(){
	if(ajaxRequest.readyState == 4){
		...
		alldata = resp.split("$$$");
		//alert(alldata);
		dept_idname_add_payment = alldata[0].split("===");
		dept_id_add_payment = dept_idname_add_payment[0].split("***");
	    dept_name_add_payment = dept_idname_add_payment[1].split("***");
				
		document.add_payment.dept_add_payment.options.length=0;
		document.add_payment.dept_add_payment.options[0]=new Option("Select","Select", true, false);
		for (i=0; i<dept_id_add_payment.length; i++){
			document.add_payment.dept_add_payment.options[i+1]=new Option(dept_name_add_payment[i],dept_id_add_payment[i], false, false);

		}
				
		sec_idname_add_payment = alldata[1].split("===");
		sec_id_add_payment = sec_idname_add_payment[0].split("***");
		sec_name_add_payment = sec_idname_add_payment[1].split("***");
	 		
		document.add_payment.section_add_payment.options.length=0;
		document.add_payment.section_add_payment.options[0]=new Option("Select","Select", true, false); 
		for (i=0; i<sec_id_add_payment.length; i++){
			//alert(sec_name[i]);
			document.add_payment.section_add_payment.options[i+1]=new Option(sec_name_add_payment[i],sec_id_add_payment[i], false, false);

		}
		
		
		line_idname_add_payment = alldata[2].split("===");
		line_id_add_payment = line_idname_add_payment[0].split("***");
		line_name_add_payment = line_idname_add_payment[1].split("***");
		
		document.add_payment.line_add_payment.options.length=0;
		document.add_payment.line_add_payment.options[0]=new Option("Select","Select", true, false); 
		for (i=0; i<line_id_add_payment.length; i++){
			document.add_payment.line_add_payment.options[i+1]=new Option(line_name_add_payment[i],line_id_add_payment[i], false, false);

		}
		
		
		desig_idname_add_payment = alldata[3].split("===");
		desig_id_add_payment = desig_idname_add_payment[0].split("***");
		desig_name_add_payment = desig_idname_add_payment[1].split("***");
		
		document.add_payment.desig_add_payment.options.length=0;
		document.add_payment.desig_add_payment.options[0]=new Option("Select","Select", true, false); 
		for (i=0; i<desig_id_add_payment.length; i++){
			document.add_payment.desig_add_payment.options[i+1]=new Option(desig_name_add_payment[i],desig_id_add_payment[i], false, false);
...
function save_add_payment()
{
 ...
 try{
   ...
   try{
	...
	var add_payment_empid = document.getElementById('add_payment_empid').value;
	var add_payment_id = document.getElementById('add_payment_id').value;

	//var deduction_ad = document.getElementById('add_deduct_deduction').value;
	
	var add_payment_year = document.getElementById('add_payment_year').value;
	var add_payment_month = document.getElementById('add_payment_month').value;

	var dept_add_payment = document.getElementById('dept_add_payment').value;

	var section_add_payment = document.getElementById('section_add_payment').value;

	var line_add_payment  = document.getElementById('line_add_payment').value;

	var desig_add_payment = document.getElementById('desig_add_payment').value;
	
	
	if (dept_add_payment !="Select")
	{
		var column_add_payment="dept_add_payment_id";
		var column_value_add_payment=dept_add_payment;
	}
	
	else if(section_add_payment !="Select")
	{
		var column_add_payment="section_add_payment_id";
		var column_value_add_payment=section_add_payment;
	}
 	else if(line_add_payment !="Select")
	{
		var column_add_payment="line_add_payment_id" ;
		var column_value_add_payment=line_add_payment;
	}
	else if(desig_add_payment !="Select")
	{
		var column_add_payment="desig_add_payment_id" ;
		var column_value_add_payment=desig_add_payment;
	}
...
function save_add_deduction()
{
 ...
 try{
   ...
   try{
	...
	var empid_ad = document.getElementById('add_deduction_empid').value;
	var tex_ad = document.getElementById('add_deduct_tex').value;
	var deduction_ad = document.getElementById('add_deduct_deduction').value;
	var year_ad = document.getElementById('add_deduct_year').value;
	var month_ad = document.getElementById('add_deduct_month').value;
	var dept_ad = document.getElementById('dept_add_deduct').value;
	var section_ad = document.getElementById('section_add_deduct').value;
	var line_ad = document.getElementById('line_add_deduct').value;
	var desig_ad = document.getElementById('desig_add_deduct').value;
	
	
	if (dept_ad !="Select")
	{
		var column_ad="emp_dept_id_ad";
		var column_value_ad=dept_ad;
	}
	
	else if(section_ad !="Select")
	{
		var column_ad="emp_sec_id_ad";
		var column_value_ad=section_ad;
	}
 	else if(line_ad !="Select")
	{
		var column_ad="emp_line_id_ad" ;
		var column_value_ad=line_ad;
	}
	else if(desig_ad !="Select")
	{
		var column_ad="emp_desi_id_ad" ;
		var column_value_ad=desig_ad;
	}
...
function work_off_delete_function()
{
...
// if(okyes==false) return;
 ...
 var work_off_day 	= document.getElementById('work_off_day').value;
...
var ajaxRequest;  // The variable that makes Ajax possible!
 ...
 var holiday_start_day 	= document.getElementById('holiday_start_day').value;
 var holiday_end_date 	= document.getElementById('holiday_end_date').value;
...
// alert(start_leave_date);
var queryString="holiday_start_day="+holiday_start_day+"&holiday_end_date="+holiday_end_date+"&holiday_description="+holiday_description;
...
function work_off_save_function()
{
...
emp_id= document.getElementById('job_card_emp_id').value;
            ...
            win = desktop.createWindow({
                ...
                items:
                    new Ext.TabPanel({
                        ...
                        items: [{
                            title: 'Attendence Entry',
                            header:false,
							html : "<br/><form name='entry'><table width='100%' border='0' cellpadding='5' cellspacing='0'><tr><th colspan='2' scope='col' align='center'><strong> Attendence Entry</strong></th></tr><tr><td colspan='3'><hr></td></tr><tr><td width='50%'><div style='width:100%; height:auto; overflow:hidden; margin:0 auto; border-right:#000 1px solid;'><table width='300' border='0' cellpadding='10'cellspacing='5'><tr><td>In Time </td><td>:</td><td><input type='radio' name='in_time' id='in_time' value='in' size='16' onclick='intime()' ></td></tr><tr><td>Out Time </td><td>:</td><td><input type='radio' name='in_time' id='in_time' size='16' onclick='intime()'></td></tr><tr><td width='42%'>Employee ID</td><td width='5%'>:</td><td width='53%'><input type='text' name='empid' id='empid' size='16'></td></tr><tr><td>Time </td><td>:</td><td><input type='text' name='time' id='time' size='16'></td></tr><tr><td>Start Date </td><td>:</td><td><input type='text' name='startdate' id='startdate' size='16'></td></tr><tr><td>End Date </td><td>:</td><td><input type='text' name='enddate' id='enddate' size='16'></td></tr><tr><td>Department </td><td>:</td><td><select id='dept' name='dept' onchange='manual_entry_dept()'><option value=''></option></select></td></tr><tr><td>Section </td><td>:</td><td><select id='section' name='section' onchange='manual_entry_section()'><option value=''></option></select></td></tr><tr><td>Line No </td><td>:</td><td><select id='line' name='line' onchange='manual_entry_desig()'><option value=''></option></select></td> </tr><tr><td>Designation </td><td>:</td><td><select id='desig' name='desig' onchange='manual_entry_desig()'><option value=''></option></select></td></tr><tr><td></td><td></td><td><input type='button' name='btn' id='btn' onclick='manual_entry_save()' value='Save' size='15'></td></tr></table></div></td><td width='50%'><div style='width:100%; height:200px; margin:0 auto;'></div></td></tr></table></form>Note : Date Format [YYYY-MM-DD] Time format [HH:MM:SS]",
                            border:false
                        },{
                            title: 'Deduct Tax and Other',
                            header:false,
                            html : "<br/><form name='add_deduction'><table width='100%' border='0' cellpadding='5' cellspacing='0'><tr><th colspan='2' scope='col' align='center'><strong>Deduct Tax and Other Entry</strong></th></tr><tr><td colspan='3'><hr></td></tr><tr><td width='50%'><div style='width:100%; height:auto; overflow:hidden; margin:0 auto; border-right:#000 1px solid;'><table width='300' border='0' cellpadding='10'cellspacing='5'><tr><td width='42%'>Employee ID</td><td width='5%'>:</td><td width='60%'><input type='text' name='add_deduction_empid' id='add_deduction_empid' size='16'></td></tr><tr><td>Tex Amount</td><td>:</td><td><input type='text' name='add_deduct_tex' id='add_deduct_tex' size='16'></td></tr><tr><td>Deduction Amount</td><td>:</td><td><input type='text' name='add_deduct_deduction' id='add_deduct_deduction' size='16'></td></tr><tr><td>Year</td><td>:</td><td><select id='add_deduct_year'><option value='2011'>2011</option><option value='2012'>2012</option><option value='2013'>2013</option><option value='2014'>2014</option><option value='2015'>2015</option><option value='2016'>2016</option><option value='2017'>2017</option><option value='2018'>2018</option><option value='2019'>2019</option><option value='2020'>2020</option><option value='2021'>2021</option><option value='2022'>2022</option></select></td></tr><tr><td>Month </td><td>:</td><td><select id='add_deduct_month'><option value='01'>January</option><option value='02'>February</option><option value='03'>March</option><option value='04'>April</option><option value='05'>May</option><option value='06'>Jun</option><option value='07'>July</option><option value='08'>August</option><option value='09'>September</option><option value='10'>October</option><option value='11'>November</option><option value='12'>December</option></select></td></tr><tr><td>Department </td><td>:</td><td><select id='dept_add_deduct' name='dept_add_deduct' ><option value=''></option></select></td></tr><tr><td>Section </td><td>:</td><td><select id='section_add_deduct' name='section_add_deduct' ><option value=''></option></select></td></tr><tr><td>Line No </td><td>:</td><td><select id='line_add_deduct' name='line_add_deduct' ><option value=''></option></select></td> </tr><tr><td>Designation </td><td>:</td><td><select id='desig_add_deduct' name='desig_add_deduct'><option value=''></option></select></td></tr><tr><td></td><td></td><td><input type='button' name='btn' id='btn' onclick='save_add_deduction()' value='Save' size='15'><input type='button' onclick='add_deduction_add()' value='All'/></td></tr></table></div></td><td width='50%'><div style='width:100%; height:200px; margin:0 auto;'></div></td></tr></table></form>Note : Date Format [YYYY-MM-DD] Time format [HH:MM:SS]",
                            border:false
                        },{
                            title: 'Others Payment',
                            header:false,
                            html : "<br/><form name='add_payment'><table width='100%' border='0' cellpadding='5' cellspacing='0'><tr><th colspan='2' scope='col' align='center'><strong>Others Payment</strong></th></tr><tr><td colspan='3'><hr></td></tr><tr><td width='50%'><div style='width:100%; height:auto; overflow:hidden; margin:0 auto; border-right:#000 1px solid;'><table width='300' border='0' cellpadding='10'cellspacing='5'><tr><td width='42%'>Employee ID</td><td width='5%'>:</td><td width='60%'><input type='text' name='add_payment_empid' id='add_payment_empid' size='16'></td></tr><tr><td>Amount</td><td>:</td><td><input type='text' name='add_payment_id' id='add_payment_id' size='16'></td></tr><tr><td>Year</td><td>:</td><td><select id='add_payment_year'><option value='2011'>2011</option><option value='2012'>2012</option><option value='2013'>2013</option><option value='2014'>2014</option><option value='2015'>2015</option><option value='2016'>2016</option><option value='2017'>2017</option><option value='2018'>2018</option><option value='2019'>2019</option><option value='2020'>2020</option><option value='2021'>2021</option><option value='2022'>2022</option></select></td></tr><tr><td>Month </td><td>:</td><td><select id='add_payment_month'><option value='01'>January</option><option value='02'>February</option><option value='03'>March</option><option value='04'>April</option><option value='05'>May</option><option value='06'>Jun</option><option value='07'>July</option><option value='08'>August</option><option value='09'>September</option><option value='10'>October</option><option value='11'>November</option><option value='12'>December</option></select></td></tr><tr><td>Department </td><td>:</td><td><select id='dept_add_payment' name='dept_add_payment' ><option value=''></option></select></td></tr><tr><td>Section </td><td>:</td><td><select id='section_add_payment' name='section_add_payment' ><option value=''></option></select></td></tr><tr><td>Line No </td><td>:</td><td><select id='line_add_payment' name='line_add_payment' ><option value=''></option></select></td> </tr><tr><td>Designation </td><td>:</td><td><select id='desig_add_payment' name='desig_add_payment'><option value=''></option></select></td></tr><tr><td></td><td></td><td><input type='button' name='btn' id='btn' onclick='save_add_payment()' value='Save' size='15'><input type='button' onclick='add_payment_add()' value='All'/></td></tr></table></div></td><td width='50%'><div style='width:100%; height:200px; margin:0 auto;'></div></td></tr></table></form>Note : Date Format [YYYY-MM-DD] Time format [HH:MM:SS]",
                            border:false
                        },{
                            title: 'Present to Absent',
                            header:false,
                            html : "<form name='present_absent'><table width='100%' border='0' cellpadding='5' cellspacing='0'><tr><th colspan='2' scope='col' align='center'><strong>Present to Absent</strong></th></tr><tr><td colspan='3'><hr></td></tr><tr><td width='50%'><div style='width:100%; height:auto; overflow:hidden; margin:0 auto; border-right:#000 1px solid;'><table width='300' border='0' cellpadding='10'cellspacing='5'><tr><td width='42%'>Employee ID</td><td width='5%'>:</td><td width='60%'><input type='text' name='empid_present_absent' id='empid_present_absent' size='16'></td></tr><tr><td>Date</td><td>:</td><td><input type='text' name='startdate_present_absent' id='startdate_present_absent' size='16'></td></tr><tr><td></td><td></td><td><input type='button' onclick='manual_entry_Delete()' value='DELETE'/></td></tr></table></div></td><td width='50%'><div style='width:100%; height:200px; margin:0 auto;'></div></td></tr></table></form>Note : Date Format [YYYY-MM-DD] Time format [HH:MM:SS]",
                            border:false
                        },{
                            title: 'Holiday Entry',
                            header:false,
               html : "<br/><form  name='holiday' ><table width='100%' border='0' align='center' style='padding:10px'><tr><td><input type='hidden' size='40px' name='holiday_name' id='holiday_name' ></td></tr><tr><td width='20%'>Start Date</td><td><input type='text' size='20px' id='holiday_start_day'></td></tr><tr><td width='20%'>End Date</td><td><input type='text' size='20px' id='holiday_end_date'></td></tr><tr><td width='20%'>Description</td><td><input type='text' size='20px' id='holiday_description'></td></tr><br/><tr><td align='right' width='20%'></td><td> &nbsp;<input type='button' name='holiday_save_id'  onclick='save_holiday()' value='SAVE'/>&nbsp;</td></tr><tr></table></form>",
                            border:false
                        },{
                            title: 'Work off Entry',
                            header:false,
               html : "<br/><form name='work_off'><table width='100%' border='0' cellpadding='5' cellspacing='0'><tr><th colspan='2' scope='col' align='center'><strong>Work off</strong></th></tr><tr><td colspan='3'><hr></td></tr><tr><td width='50%'><div style='width:100%; height:auto; overflow:hidden; margin:0 auto; border-right:#000 1px solid;'><table width='300' border='0' cellpadding='10'cellspacing='5'><tr><td>Date</td><td>:</td><td><input type='text' name='work_off_date' id='work_off_date' size='16'></td></tr><tr><td width='42%'>Employee ID</td><td width='5%'>:</td><td width='60%'><input type='text' name='work_off_empid' id='work_off_empid' size='16'></td></tr><tr><td>Start</td><td>:</td><td><select name='work_off_start' id='work_off_start' onchange='add_work_off()' /><option value='select'>Select</option><option value='all'>ALL</option></select></td></tr><tr><td>Department </td><td>:</td><td><select id='dept_work_off' name='dept_work_off' ><option value=''></option></select></td></tr><tr><td>Section </td><td>:</td><td><select id='section_work_off' name='section_work_off' ><option value=''></option></select></td></tr><tr><td>Line No </td><td>:</td><td><select id='line_work_off' name='line_work_off' ><option value=''></option></select></td> </tr><tr><td>Designation </td><td>:</td><td><select id='desig_work_off' name='desig_work_off'><option value=''></option></select></td></tr><tr><td></td><td></td><td><input type='button' name='btn' id='btn' onclick='save_work_off()' value='Save' size='15'></td></tr></table></div></td><td width='50%'><div style='width:100%; height:200px; margin:0 auto;'></div></td></tr></table></form>Note : Date Format [YYYY-MM-DD] Time format [HH:MM:SS]",
                            border:false
                        }]
                    })
            });
    ...
    createWindow : function(src){
        ...
        if(!win){
            win = desktop.createWindow({
                ...
                width:650,
                height:450,
                html : "<form  name='salary_report'><br/><div><div align='center'><fieldset style='width:500px;'><legend><font size='+1'><b>Process</b></font></legend>Select Month and Year :<select id='report_month_sal'><option value='01'>January</option><option value='02'>February</option><option value='03'>March</option><option value='04'>April</option><option value='05'>May</option><option value='06'>Jun</option><option value='07'>July</option><option value='08'>August</option><option value='09'>September</option><option value='10'>October</option><option value='11'>November</option><option value='12'>December</option></select><select id='report_year_sal'><option value='2011'>2011</option><option value='2012'>2012</option><option value='2013'>2013</option><option value='2014'>2014</option><option value='2015'>2015</option><option value='2016'>2016</option><option value='2017'>2017</option><option value='2018'>2018</option><option value='2019'>2019</option><option value='2020'>2020</option></select><input type='button' name='view' onclick='salary_process()' value='Process'/></fieldset></div></div><div align='center'><fieldset style='width:500px;'><legend><font size='+1'><b>Category Options</b></font></legend><table><tr><td>Start</td><td>:</td><td><select name='salary_start' id='salary_start' onchange='salary_get_data()' /><option value='select'>Select</option><option value='all'>ALL</option></select></td></tr><tr><td>Department </td><td>:</td><td><select id='salary_dept' name='salary_dept' ><option value=''></option></select></td><td>Section </td><td>:</td><td><select id='salary_section' name='salary_section' ><option value=''></option></select></td></tr><tr><td>Line No </td><td>:</td><td><select id='salary_line' name='salary_line'><option value=''></option></select></td><td>Designation </td><td>:</td><td><select id='salary_desig' name='salary_desig'><option value=''></option></select></td></tr></table></fieldset></div><div align='center'><br><fieldset style='width:500px;'><legend><font size='+1'><b>Monthly Report</b></font></legend><br>Chose Emp Status :<select id='salary_emp_status' name='salary_emp_status'><option value='1'>Reguler</option><option value='2'>New</option><option value='3'>left</option><option value='4'>Resign</option><option value='5'>Probation</option></select><br><input type='button' onclick='monthly_salary_sheet()' value='Monthly Salary Sheet'/><input type='button' onclick='salary_summary_report()' value='Salary Summary Report'/><input type='button' onclick='' value='AL Calculation'/><input type='button' onclick='' value='Festival Bonus Sheet(% of Gross)'/><input type='button' onclick='' value='PF'/><input type='button' onclick='' value='Line Wise Salary Summary'/><input type='button' onclick='' value='Salay Summary (R,N)'/><input type='button' onclick='' value='Salay Summary (R,N,D)'/></fieldset></div><div align='center'><br><fieldset style='width:500px;'><legend><font size='+1'><b>Pay Slip Report</b></font></legend><br>Emp ID :<input type='text' id='pay_id_start' name='pay_id_start'> TO <input type='text' id='pay_id_end' name='pay_id_end'><br><input type='button' onclick='payslip_report()' value='Pay Slip'/></fieldset></div></from>",
                iconCls: 'bogus',
                shim:false,
                animCollapse:false,
                constrainHeader:true
...
MyDesktop.otherReport = Ext.extend(Ext.app.Module, {
	id:'other-report',
    init : function(){
        this.launcher = {
            text: 'Others Report',
    ...
    createWindow : function(src){
        var desktop = this.app.getDesktop();
        var win = desktop.getWindow('other-report');
        if(!win){
            win = desktop.createWindow({
                id: 'other-report',
                title:src.text,
                width:650,
                height:450,
                html : "<form  name='other_report'><br/><div><div align='center'><fieldset style='width:620px;'><legend><font size='+1'><b>Options</b></font></legend> Enter date : <input type='text' id='other_start_date' /> TO <input type='text' id='other_end_date' /><br>Example: YYYY-MM-DD TO YYYY-MM-DD<br><table><tr><td>Start</td><td>:</td><td><select name='other_start' id='other_start' onchange='other_get_data()' /><option value='select'>Select</option><option value='all'>ALL</option></select></td></tr><tr><td>Department </td><td>:</td><td><select id='other_dept' name='other_dept' ><option value=''></option></select></td><td>Section </td><td>:</td><td><select id='other_section' name='other_section' ><option value=''></option></select></td></tr><tr><td>Line No </td><td>:</td><td><select id='other_line' name='other_line'><option value=''></option></select></td><td>Designation </td><td>:</td><td><select id='other_desig' name='other_desig'><option value=''></option></select></td></tr></table></fieldset></div></div><div align='center'><br><br><fieldset style='width:620px;'><legend><font size='+1'><b>Reports</b></font></legend><br><input type='button' onclick='left_emp_report()' value='Left Employee Report'/><input type='button' onclick='resign_emp_report()' value='Resign Employee Report'/><input type='button' onclick='join_emp_report()' value='New Join Employee Report'/></fieldset></div></from>",
                iconCls: 'bogus',
                shim:false,
                animCollapse:false,
                constrainHeader:true
