			...
			foreach($query->result() as $rows)
			{
				...
				if($sp_eligibility == true)
				{
					...
					if($inc_prom->num_rows() > 0 ) {
						...
						$gross_sal_com 	= $rowsss->new_com_salary;
						$emp_dept_id	= $rowsss->new_dept;
						$emp_grade_id	= $rowsss->new_grade;
						$emp_sec_id 	= $rowsss->new_section;
						$emp_line_id	= $rowsss->new_line;
						$desi_id 		= $rowsss->new_desig;
					}else{
						$where = "trim(substr(effective_month,1,7)) > '$year_month'";
						$this->db->select("*");
						$this->db->where("new_emp_id",$emp_id);
					...
					if (!empty($attendances->com_ot) && $rows->com_ot_entitle != 1) {
						...
						$ot_amount = $attendances->com_ot * $ot_ratec;
					} else {
						$ot_ratec = 0;
						$ot_hour = 0;
						$ot_amount = 0;
					...
					//EXTRA OT CALCULATION
    				if (!empty($ot_hour)) {
						$collect_eot_hour = $attendances->com_eot;
						$ot_eot_12am_hour = $attendances->ot_eot_12am;
						$ot_eot_4pm_hour  = $attendances->ot_eot_4pm;
						$modify_eot_hour  = $attendances->modify_eot;
