			              	...
			              	<div class="form-group">
								...
								<select name="status" id="status" class="form-control input-sm" onChange="grid_emp_list()">
				                	...
				                	<?php foreach ($categorys as $key => $row) { ?>
					                  <option value="<?= $row->id ?>" <?= ($row->id==1)?'selected':'' ?> ><?= $row->status_type; ?></option>
				                	<?php } ?>
				                </select>
				...
				<div style="height: 80vh; overflow-y: scroll; background: #fff !important">
					<table class="table table-hover" id="fileDiv">
						<tr style="position: sticky;top: 0;z-index:1">
					  	...
					  	?>
						  	<tr id="removeTr">
						      <td><input type="checkbox" class="checkbox" id="emp_id" name="emp_id[]" value="<?= $emp->id ?>"></td>
	        ...
	        success: function(response){
		        ...
		        if (response.length != 0) {
		            ...
		            $.each(response, function(index,value) {
		              ...
		              items += '<td><input type="checkbox" class="checkbox" id="select_emp_id" name="select_emp_id[]" value="'+value.id+'" ></td>';
